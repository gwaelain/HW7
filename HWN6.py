...
class Lecturer(Mentor):
    ...
    def __str__(self):
        grades_count = 0
        for lecturer in self.grades:
            grades_count += len(self.grades[lecturer])
        self.average_rating = sum(map(sum, self.grades.values())) / grades_count
...
lecturer_1 = Lecturer('Art', 'Zlov')
lecturer_1.courses_attached += ['Python']

lecturer_2 = Lecturer('Olli', 'Perov')
...
reviewer_2 = Reviewer('Ris', 'Dar')
reviewer_2.courses_attached += ['Python']
reviewer_2.courses_attached += ['Java']

student_1 = Student("Ann", "Gor")
student_1.courses_in_progress += ['Python']
student_1.finished_courses += ['Основы программирования']

student_2 = Student('Lumin', 'Gurz')
...
print()
print()
print(f'Результат сравнения студентов (по средним оценкам за ДЗ): ' f'{student_1.name} {student_1.surname} < {student_2.name} {student_2.surname} = {student_1 > student_2}')
print()